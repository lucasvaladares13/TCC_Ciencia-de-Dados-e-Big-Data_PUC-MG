...
dt_temporada = pd.read_excel(
    path_DB + 'dt_temporada.xlsx', dtype={'Equipe_ID': np.int32})
...
def calculate_score(dt_time_sts):
    dt_time_sts['Diferenca'] = dt_time_sts['GolsFeitos'] - \
        dt_time_sts['GolsSofridos']
...
def merge_time(df_partida, df_time, sufix):
    ...
    df_partida['Partida_Equipe_ID'] = df_partida[sufix +
                                                 'LastPartida_ID'].astype(str) + "_" + df_partida[sufix+'Equipe_ID'].astype(str)
    df_time['Partida_Equipe_ID'] = df_time['Partida_ID'].astype(
        str) + "_" + df_time['Equipe_ID'].astype(str)
    ...
    df_partida = pd.merge(df_partida, df_time,
                          how='inner', on='Partida_Equipe_ID')
...
def merge_time_atual(df_partida, df_time, sufix, Param):
    ...
    df_partida['Partida_Equipe_ID'] = df_partida['Partida_ID'].astype(
        str) + "_" + df_partida[sufix+'Equipe_ID'].astype(str)
    df_time['Partida_Equipe_ID'] = df_time['Partida_ID'].astype(
        str) + "_" + df_time['Equipe_ID'].astype(str)
    df_time = df_time.drop(['Partida_ID', 'Equipe_ID'], axis='columns')
    ...
    df_partida = pd.merge(df_partida, df_time, how='left',
                          on='Partida_Equipe_ID')
    df_partida = df_partida.rename(columns=dict_columns)
...
def merge_aux(df_partida, df_temporada, sufix):
    ...
    df_temporada['key'] = df_temporada['Equipe_ID'].astype(
        str)+"_"+df_temporada['ano'].astype(str)+"_"+df_temporada['info_type'].astype(str)
    df_temporada = df_temporada[list_col]
    df_partida['ano_anterior'] = df_partida['ano_ref'] - 1
    df_partida[sufix+'key'] = df_partida[sufix+'Equipe_ID'].astype(
        str)+"_"+df_partida['ano_anterior'].astype(str)+"_"+type_info
    nCol = sufix+'key'
    df_partida = pd.merge(df_partida, df_temporada,
                          how='inner', left_on=nCol, right_on='key')
...
def merge_ID(df_partida, df_LastPartida, sufix):
    df_partida['key'] = df_partida['Partida_ID'].astype(
        str) + "_" + df_partida[sufix+'Equipe_ID'].astype(str)
...
def get_last_partidaID(df_partida, df_info):
    ...
    df = df_partida[['Partida_ID', 'm_Equipe_ID', 'Data']].rename(
        columns={'m_Equipe_ID': 'Equipe_ID'})
    df = df.append(df_partida[['Partida_ID', 'v_Equipe_ID', 'Data']].rename(
        columns={'v_Equipe_ID': 'Equipe_ID'}))
    ...
    for ID in list(set(df.Equipe_ID.tolist())):
        ...
        # print(df_Equipe)
    df_PartidaID['key'] = df_PartidaID['Partida_ID'].astype(
        str) + "_" + df_PartidaID['Equipe_ID'].astype(str)
    df_PartidaID = df_PartidaID[['key', 'LastPartida_ID']]
    df_PartidaID['LastPartida_ID'] = df_PartidaID['LastPartida_ID'].astype(
...
df_sts = calculate_mdmv(10, ['GolsFeitos', 'GolsSofridos', 'Faltas', 'Escanteios', 'Cruzamentos',
                             'QtdDefesas', 'Impedimentos', 'posse', 'chutesaogol'], dt_time_sts, dt_info, dt_time)
...
# ETAPA ADICIONADA APOS A ANALISE DE DADOS
dt_partida.loc[dt_partida.m_MdPosse == 0,
               'm_MdPosse'] = dt_partida.loc[dt_partida.m_MdPosse != 0, 'm_MdPosse'].mean()
dt_partida.loc[dt_partida.v_MdPosse == 0,
               'v_MdPosse'] = dt_partida.loc[dt_partida.v_MdPosse != 0, 'v_MdPosse'].mean()
